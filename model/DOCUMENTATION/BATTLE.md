...
Finally, it will count down all the status changes and remove those that have ended, on the *new* open participant. This means status progression is parsed *at the start of the participant's turn*.
...
Hit/Evasion, as the name suggests, goes both ways. For example, Sukukaja improves both your chances to hit an enemy, and also to dodge incoming attacks. As such, parsing the Hit/Evasion bonus runs through both the caster and the target's `dodgestatus` lists.
